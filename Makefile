...
all: clean
	...
	docker run -it --rm --name my-running-script -v "$$PWD":/usr/src/myapp -w /usr/src/myapp python:3 sh -c 'pip install -r requirements.txt && python pinlinks.py -A $(PINBOARD_TOKEN) -f $(FROM_DATE) -t $(TO_DATE) -N $(NO_RANDOM_COVER_IMAGE) | tee $(OUTPUT_DIR)/$(TITLE).md'
